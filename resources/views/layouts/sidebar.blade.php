    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-white border-r border-slate-200 fixed h-full transition-all duration-300">
            ...
            <nav class="mt-4 px-4 space-y-2">
                <a href="{{ route('dashboard.index') }}" class="flex items-center gap-3 px-4 py-3 text-blue-600 bg-blue-50 rounded-xl font-medium">
                    <i class="fas fa-home"></i> Dashboard
                </a>
                <a href="{{ route('pelanggan.index') }}" class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-50 hover:text-blue-600 rounded-xl transition">
                    <i class="fas fa-users"></i> Data Pelanggan
                </a>
                <a href="{{ route('pembayaran.index') }}" class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-50 hover:text-blue-600 rounded-xl transition">
                    <i class="fas fa-file-invoice-dollar"></i> Pembayaran Air
                </a>
                <a href="#" class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-50 hover:text-blue-600 rounded-xl transition">
            ...
            <div class="absolute bottom-0 w-full p-4 border-t border-slate-100">
                <div class="flex items-center gap-3 px-2 py-2">
                    ...
                    <button class="text-slate-400 hover:text-red-500"><i class="fas fa-sign-out-alt"></i></button>
                </div>
            </div>
        </aside>

        <main class="flex-1 ml-64">
            <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 flex items-center justify-between px-8 z-10">
